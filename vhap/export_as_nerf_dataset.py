...
class NeRFDatasetWriter:
    def __init__(self, cfg_data: DataConfig, tgt_folder: Path, subset:Optional[str]=None, scale_factor: Optional[float]=None, background_color: Optional[str]=None, device_id: int = 0):
    ...
    def write(self):
        ...
        for i, item in tqdm(enumerate(self.dataloader), total=len(self.dataloader)):
            ...
            transform_matrix = torch.cat([extrinsic, torch.tensor([[0,0,0,1]], device=self.device)], dim=0)
            transform_matrix_np = transform_matrix.cpu().numpy()

            intrinsic = item['intrinsic'].double()
            intrinsic_np = intrinsic.cpu().numpy()

            cx = intrinsic_np[0, 2]
            cy = intrinsic_np[1, 2]
            fl_x = intrinsic_np[0, 0]
            fl_y = intrinsic_np[1, 1]
            h = item['rgb'].shape[0]
            w = item['rgb'].shape[1]
            ...
            frame_item = {
                ...
                "transform_matrix": transform_matrix_np.tolist(),

                "file_path": f"images/{item['timestep_index']:05d}_{item['camera_index']:02d}.png",
            }

            # Keep data on GPU until writing
            path2data = {
...
class TrackedFLAMEDatasetWriter:
    def __init__(self, cfg_model: ModelConfig, src_folder: Path, tgt_folder: Path, mode: Literal['mesh', 'param'], epoch: int = -1, device_id: int = 0):
        print("---- Config: model ----")
        ...
        self.flame_model = FlameHead(cfg_model.n_shape, cfg_model.n_expr, add_teeth=True).to(self.device)
    
    def relocate_flame_meshes(self, flame_param):
        """ Relocate FLAME to the origin and return the transformation matrix to modify camera poses. """
        Ts = torch.tensor(flame_param['translation']).to(self.device)
        T_mean = Ts.mean(0)
        M = torch.eye(4, device=self.device)
        M[:3, 3] = -T_mean
        flame_param['translation'] = (M[:3, 3] + Ts).cpu().numpy()
        return M.cpu().numpy()
    ...
    def write(self):
        if self.mode == 'mesh':
            ...
            verts = infer_flame_params(self.flame_model, self.flame_params, indices, device_id=int(self.device.split(':')[-1]))
    ...
    def write_canonical_mesh(self):
        ...
        if 'static_offset' in self.flame_params:
            static_offset = torch.tensor(self.flame_params['static_offset']).to(self.device)
        else:
            static_offset = None
        with torch.no_grad():
            ret = self.flame_model(
                torch.tensor(self.flame_params['shape']).to(self.device)[None, ...],
                torch.zeros(*self.flame_params['expr'][:1].shape, device=self.device),
                torch.zeros(*self.flame_params['rotation'][:1].shape, device=self.device),
                torch.zeros(*self.flame_params['neck_pose'][:1].shape, device=self.device),
                torch.tensor([[0.3, 0, 0]], device=self.device),
                torch.zeros(*self.flame_params['eyes_pose'][:1].shape, device=self.device),
                torch.zeros(*self.flame_params['translation'][:1].shape, device=self.device),
        ...
        obj_data = get_obj_content(verts[0].cpu(), self.flame_model.faces)
    ...
    def write_expr_and_mesh(tgt_folder, exp_path, expr, mesh_path, verts, faces):
        ...
        obj_data = get_obj_content(verts.cpu(), faces)
...
class MaskFromFLAME:
    def __init__(self, cfg_model: ModelConfig, tgt_folder, background_color: str, device_id: int = 0) -> None:
        self.device = f'cuda:{device_id}'
        background_color = self.cfg_data.background_color if background_color is None else background_color
        if background_color == 'white':
            self.background_tensor = torch.tensor([255, 255, 255]).byte().to(self.device)
        elif background_color == 'black':
            self.background_tensor = torch.tensor([0, 0, 0]).byte().to(self.device)
        else:
    ...
    def write(self):
        ...
        for i, frame in enumerate(tqdm(self.dataloader)):
            ...
            if timestep not in t2verts:
                t2verts[timestep] = infer_flame_params(self.flame_model, frame['flame_param'], [0]).to(self.device)
            verts = t2verts[timestep]

            # render to get forground mask
            RT = frame['extrinsics'].to(self.device)[None]
            K = frame['intrinsics'].to(self.device)[None]
            ...
            img = frame['image'].to(self.device)
            img = img * mask[:, :, None] + self.background_tensor[None, None, :] * (1-mask)[:, :, None]
            ...
            if 'fg_mask_path' in frame and 'fg_mask' in frame:
                fg_mask = frame['fg_mask'].to(self.device)
    ...
    def get_mask(self, verts, RT, K, h, w):
        faces = self.flame_model.faces.to(self.device)
        ...
        xy = verts_ndc[:, :2]
        xy[:, 1] = -xy[:, 1]
        xy = (xy * 0.5 + 0.5) * torch.tensor([[h, w]], device=self.device)
    ...
    def get_mask_tilted_line(self, verts, RT, K, h, w):
        ...
        verts_xy = (verts_xy * 0.5 + 0.5) * torch.tensor([w, h], device=self.device)
...
def infer_flame_params(flame_model: FlameHead, flame_params: Dict, indices:List, device_id: int = 0):
    device = f'cuda:{device_id}'
    if 'static_offset' in flame_params:
        static_offset = flame_params['static_offset']
        if isinstance(static_offset, np.ndarray):
            static_offset = torch.tensor(static_offset).to(device)
    else:
        static_offset = None
    for k in flame_params:
        if isinstance(flame_params[k], np.ndarray):
            flame_params[k] = torch.tensor(flame_params[k]).to(device)
    with torch.no_grad():
        ret = flame_model(
...
def main(
    ...
    nerf_dataset_writer = NeRFDatasetWriter(cfg.data, tgt_folder, subset, scale_factor, background_color, device_id)
    nerf_dataset_writer.write()

    flame_dataset_writer = TrackedFLAMEDatasetWriter(cfg.model, src_folder, tgt_folder, mode=flame_mode, epoch=epoch, device_id=device_id)
    flame_dataset_writer.write()

    if create_mask_from_mesh:
        mask_generator = MaskFromFLAME(cfg.model, tgt_folder, background_color, device_id=device_id)
