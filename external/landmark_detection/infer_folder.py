...
if __name__ == '__main__':
    ...
    det_model_path = os.path.join(project_root, 'model_zoo', 'flame_tracking_models', 'FaceBoxesV2.pth')
    detector = FaceBoxesDetector('FaceBoxes', det_model_path, use_gpu, device)

    ########### facial alignment ############
    model_path = os.path.join(project_root, 'model_zoo', 'flame_tracking_models', '68_keypoints_model.pkl')
    ...
    for file_name in img_path_list:
        ...
        for idx in range(len(detections)):
            ...
            # Create landmark directory and save annotated image
            save_path = os.path.join(os.path.dirname(args.folder_path), 'landmark')
            os.makedirs(save_path, exist_ok=True)
            cv2.imwrite(os.path.join(save_path, file_name), image_draw)
        
    # Save keypoint.json in the input folder itself
    json.dump(kpts_code, open(os.path.join(args.folder_path, 'keypoint.json'), 'w'))
