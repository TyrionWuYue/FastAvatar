...
class ConstantLR(LRScheduler):
    """Constant Learning Rate Scheduler that maintains the original learning rates from optimizer groups."""
    
    def __init__(self, optimizer, last_iter: int = -1):
        """
        Args:
            optimizer: The optimizer to schedule
            last_iter: The index of last iteration (default: -1)
        """
        super().__init__(optimizer, last_iter)
        logger.info(f"======== Using ConstantLR with original learning rates ========")

    def get_lr(self):
        """Return the original learning rates for all parameter groups."""
