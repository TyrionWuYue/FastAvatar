...
class VFHQDataset(FrameBaseDataset):
    ...
    def __init__(self, root_dirs: str,
        ...
        self.is_val = is_val
        self.use_random_input = kwargs.get("use_random_input", True)
    ...
    def _extract_id_from_path(path: str) -> str:
        """Extract ID from path. For VFHQ, ID is the first part of path."""
        # path format: "00000/EXP-0-random" or "00000"
        parts = path.split('/')
    ...
    def _split_train_val(self, val_num: int, val_id: Optional[list]):
        ...
        if self.is_val:
            if len(val_uid_indices) > val_num:
                val_uid_indices = random.sample(val_uid_indices, val_num)
            self.uids = [self.uids[i] for i in val_uid_indices]
            print(f"Validation set: {len(self.uids)} samples from {len(selected_ids)} IDs")
        else:
    ...
    def load_processed_data(self, frame_dir):
        """Load all processed data from processed_data/<frame_idx>/ directory."""
    ...
    def inner_get_item(self, idx):
        ...
        # path format for VFHQ: "person_id/camera_id/sequence_name/group_id"
        # Since VFHQ is monocular, camera_id is "cam_000000000" and sequence_name is like "EXP-0-random"

        camera_frame_pairs = frame_data["data"]
        input_frames = frame_data.get("input_frames", self.max_input_frames)

        # Extract person_id from path: "person_id/group_id"
        path_parts = path.split('/')
        if len(path_parts) >= 1:
            person_id = path_parts[0]
        else:
            raise ValueError(f"Unexpected path format for VFHQ: {path}")
        ...
        if len(camera_frame_pairs) > total_frames_needed:
            # disorder sampling if needed, or sequential
            if self.disorder:
                start_idx = np.random.randint(0, len(camera_frame_pairs) - total_frames_needed + 1)
            else:
                start_idx = 0
        ...
        for i, pair in enumerate(camera_frame_pairs):
            ...
            # Extract sequence name from pair if available, otherwise it might be implicit
            # In preprocess_dataset.py, "seq" is added to the pair dictionary
            sequence_name = pair.get("seq")
            
            if sequence_name is None:
                # Fallback if seq is missing (should not happen with new preprocess)
                raise ValueError(f"Sequence name missing in metadata for {path}")

            # Directory structure: root/person_id/camera_id/sequence_name
            # For VFHQ monocular, usually: root/person_id/cam_id/sequence_name
            camera_dir = os.path.join(self.root_dir, person_id, camera_id, sequence_name)
            
            transforms_json = os.path.join(camera_dir, "transforms.json")
            if not os.path.exists(transforms_json):
                 # Fallback for some VFHQ structures where it might be directly directly under cam or person
                 # But sticking to standard structure first
                 raise FileNotFoundError(f"transforms.json not found at {transforms_json}")

            # Build frame directory path
            frame_dir = os.path.join(camera_dir, "processed_data", f"{frame_idx:05d}")
            
            rgb, mask, intrinsic, landmarks, bg_color = self.load_processed_data(frame_dir)
            
            if len(rgb.shape) == 3: rgb = rgb.unsqueeze(0)
            if len(mask.shape) == 3: mask = mask.unsqueeze(0)
            
            # get frame info for pose
            # We need to reimplement get_frame_info logic slightly since paths differ
            with open(transforms_json) as fp:
                data = json.load(fp)
            # Find frame
            frame_info = None
            ...
            if frame_info is None:
                 # Search by name if index lookup fails
                 for f in data['frames']:
                    if str(frame_idx).zfill(5) in f.get('file_path', ''):
                        frame_info = f
                        break
        ...
        # Collate list to tensors
        c2ws = torch.stack(c2ws, dim=0)
        intrs = torch.stack(intrs, dim=0)
        rgbs = torch.cat(rgbs, dim=0)
        bg_colors = torch.tensor(np.array(bg_colors), dtype=torch.float32).unsqueeze(-1).repeat(1, 3)
        ...
        for i, pair in enumerate(camera_frame_pairs):
            ...
            flame_path = os.path.join(camera_dir, "flame_param", f"{frame_idx:05d}.npz")
            teeth_bs = None # Scan for teeth if needed
        ...
        for k, v in all_flame_params.items():
            all_flame_params[k] = torch.stack(v)
            
        # Canonical flame
        first_cam_id = camera_frame_pairs[0]["camera"]
        first_seq = camera_frame_pairs[0]["seq"]
        first_cam_dir = os.path.join(self.root_dir, person_id, first_cam_id, first_seq)
        canonical_flame_path = os.path.join(first_cam_dir, "canonical_flame_param.npz")
        if os.path.exists(canonical_flame_path):
            canonical_flame_param = self.load_flame_params(canonical_flame_path, 0, None)
            all_flame_params['betas'] = canonical_flame_param['shape'].expand(len(camera_frame_pairs), -1)
        else:
            # Fallback if canonical not found (should be there)
            pass
        ...
        for k, v in all_flame_params.items():
             ret[f'input_{k}'] = v[:input_frames]  # Use actual input_frames
             ret[f'target_{k}'] = v[-self.target_frames:]
