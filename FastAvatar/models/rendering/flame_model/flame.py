...
class FlameHeadSubdivided(FlameHead):
    ...
    def get_cano_verts(self, shape_params):
        ...
        batch_size, n_frames = shape_params.shape[:2]

        template_vertices = self.v_template_up.unsqueeze(0).unsqueeze(0).expand(batch_size, n_frames, -1, -1)
    ...
    def animation_forward(self,
        ...
        blend_expr = blend_shapes(expr, self.shapedirs_up[:, :, self.n_shape_params:])
        blend_expr = blend_expr.repeat(1, num_input_frames, 1)
        v_cano_with_expr = v_cano + blend_expr
        
        # step3. lbs
        vertices = self.skinning(v_posed=v_cano_with_expr, A=A, lbs_weights=self.lbs_weights_up, batch_size=batch_size,
                                 num_joints=self.joint_num, num_frames=num_input_frames, dtype=self.dtype, device=full_pose.device)
    ...
    def skinning(self, v_posed, A, lbs_weights, batch_size, num_joints, num_frames, dtype, device):
    ...
    def forward(
        ...
        vertices = self.skinning(v_posed=v_shaped_up, A=A, lbs_weights=self.lbs_weights_up, batch_size=batch_size,
                                 num_joints=self.joint_num, dtype=self.dtype, device=full_pose.device, num_frames=1)
