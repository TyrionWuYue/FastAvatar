...
def get_2d_sincos_pos_embed(embed_dim: int, grid_size: Union[int, Tuple[int, int]], return_grid=False) -> torch.Tensor:
    ...
    if return_grid:
        return (pos_embed.reshape(1, grid_size_h, grid_size_w, -1).permute(0, 3, 1, 2).contiguous(), grid)
    return pos_embed.reshape(1, grid_size_h, grid_size_w, -1).permute(0, 3, 1, 2).contiguous()
...
def sample_features4d(input, coords):
    ...
    return feats.permute(0, 2, 1, 3).contiguous().view(B, -1, feats.shape[1] * feats.shape[3]).contiguous()  # B C R 1 -> B R C