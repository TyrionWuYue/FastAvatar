    ...
    def forward(self, input_tensor, mask=None):
        ...
        for i in range(len(self.time_blocks)):
            ...
            if self.add_space_attn and (i % (len(self.time_blocks) // len(self.space_virtual_blocks)) == 0):
                space_tokens = tokens.permute(0, 2, 1, 3).contiguous().view(B * T, N, -1).contiguous()  # B N T C -> (B T) N C
...
class CorrBlock:
    ...
    def corr_sample(self, targets, coords):
        ...
        for i, fmaps in enumerate(self.fmaps_pyramid):
            ...
            # fmap2s: (B, S, C, H*W)
            fmap2s = fmaps.view(B, S, C, H * W).contiguous()
            ...
            corrs = corrs.view(B, S, N, H, W).contiguous()
            ...
            coords_lvl = centroid_lvl + delta_lvl.view(1, 2 * self.radius + 1, 2 * self.radius + 1, 2).contiguous()
            ...
            corrs_sampled = corrs_sampled.view(B, S, N, -1).contiguous()  # Now shape: (B, S, N, (2r+1)^2)
...
def compute_corr_level(fmap1, fmap2s, C):
    ...
    corrs = corrs.view(fmap1.shape[0], fmap1.shape[1], fmap1.shape[2], -1).contiguous()  # (B, S, N, H*W)
